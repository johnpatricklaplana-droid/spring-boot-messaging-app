...
export async function showMessagesLive (info) {
    ...
    if(chatContainer.classList.contains("show") && Number(conversationId) === Number(info.conversation_id)) {
        ...
        await fetch(`http://10.81.0.227:8080/seenMessagesLive/${currentUserId}/${info.conversation_id}`, { method: "POST" });
    }

    //get people who seen this message
    const peopleWhoSeenTheMessage = await (await fetch(`http://10.81.0.227:8080/getPeopleWhoSeenTheMessage/${info.conversation_id}`)).json();
    ...
    if (Number(info.sender) === Number(currentUserId)) {
        ...
        youProfile.style.backgroundImage = `url("http://10.81.0.227:8080/getProfilePic/${currentUserId}.png")`;
        ...
        peopleWhoSeenTheMessage.forEach(person => {
                ...
                && Number(conversationId) === Number(info.conversation_id)) 
            {
                ...
                profileOfpeopleWhoSeenTheMessage.src = `http://10.81.0.227:8080/getProfilePic/${person.user.id}.png`;
...
function appendFriendMessageLive(info, peopleWhoSeenTheMessage) {
    ...
    kachatProfile.style.backgroundImage = `url("http://10.81.0.227:8080/getProfilePic/${info.sender}.png")`;
