...
export function searchResult(peoples) {
    ...
    peoples.forEach(person => {
        ...
        span.style.backgroundImage = `url("http://10.81.0.227:8080/getProfilePic/${id}.png")`;
        li.appendChild(span);
        ul.appendChild(li);
    });
}
...
function appendCurrentUserMessage (m) {
    ...
    Object.entries(m.peopleWhoSeenTheMessage).forEach(([user_id, readStatus]) => {
        if (readStatus === "read") {
            ...
            profileOfpeopleWhoSeenTheMessage.src = `http://10.81.0.227:8080/getProfilePic/${user_id}.png`;
    ...
    youProfile.style.backgroundImage = `url("http://10.81.0.227:8080/getProfilePic/${currentUser.id}.png")`;
...
function appendFriendMessage (m) {
    ...
    kachatProfile.style.backgroundImage = `url("http://10.81.0.227:8080/getProfilePic/${m.senderId.id}.png")`;
...
export async function displayFriendList(conversationList) {
    ...
    conversationList.forEach(async conversation => {
        ...
        try {
            const lastMessageInAConversation = await (await fetch(`http://10.81.0.227:8080/getLastMessages/${conversation.id}`)).json();
            ...
            if(conversation.members.length === 2) {
                conversation.members.forEach(member => {
                    if(Number(member.id) !== Number(currentUserId)) {
                        profilepicture.style.backgroundImage = `url("http://10.81.0.227:8080/getProfilePic/${member.id}.png")`;
