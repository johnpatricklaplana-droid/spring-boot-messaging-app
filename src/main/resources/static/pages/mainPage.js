...
//         element.style.backgroundImage = `url("http://10.81.0.227:8080/getProfilePic/${id}.png")`;
//     });
...
(()=> {
    ...
    searchPeople.addEventListener("input", async ()=> {
        ...
        const url = `http://10.81.0.227:8080/search/${name}`; 
...
(() => {
    ...
    document.addEventListener("click", (event)=> {
        ...
        window.location.href = `http://10.81.0.227:8080/personprofile/${id}`;
        ...
        // personProfilePicture.style.backgroundImage = `url("http://10.81.0.227:8080/getProfilePic/${id}.png")`;
...
(() => {
    ...
    if(addButton) {
        addButton.addEventListener("click", async (event) => {
            ...
            const url = `http://10.81.0.227:8080/addFriend/${requestTo}/${requestFrom}`;
...
(() => {
    ...
    friendRequestIconAndLabelContainer.addEventListener("click", ()=> {
        window.location.href = 'http://10.81.0.227:8080/friend-request';
    });
...
(() => {
    ...
    document.addEventListener("click", async (event) => {
        if (event.target.closest(".friend") && !event.target.classList.contains("fa-ellipsis")) {
            ...
            const messages = await (await fetch(`http://10.81.0.227:8080/getMessages/${conversationId}`)).json();
            console.log(messages);
            if(conversation.members.length === 2) {
                conversation.members.forEach(member => {
                    if(Number(member.id) !== Number(currentUser.id)) {
                        friendProfile.dataset.friendId = member.id;
                        recieverProfile.style.backgroundImage = `url("http://10.81.0.227:8080/getProfilePic/${member.id}.png")`;
                        friendProfile.style.backgroundImage = `url("http://10.81.0.227:8080/getProfilePic/${member.id}.png")`;
...
window.addEventListener("load", async () => {
    ...
    const friendList = await(await fetch(
        `http://10.81.0.227:8080/getConversationList`
    )).json();
...
(() => {
    ...
    menuButtonInBottomNavBar.addEventListener("click", ()=> {
        ...
        window.location.href = `http://10.81.0.227:8080/menu`;
...
(() => {
    ...
    createGroupChat.addEventListener("click", () => {
        ...
        window.location.href = `http://10.81.0.227:8080/create-group-chat`;
