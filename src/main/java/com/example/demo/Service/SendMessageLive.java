...
public class SendMessageLive {
        ...
        public void handleTextMessageType(JsonNode messageObject, TextMessage message, Map<Integer, Set<WebSocketSession>> conversationSession) throws IOException, java.io.IOException {
    ...
    public void handleSeenMessageLive (JsonNode messageObject, TextMessage message) {
        int conversation_id = messageObject.get("conversationId").asInt();
        int user_id = messageObject.get("userId").asInt();
        messagesService.seenMessages(user_id, conversation_id);
    }
